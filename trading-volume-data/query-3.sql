...
hourly_prices AS (
  SELECT
    ...
    AVG(CASE
      ...
      THEN t.token_bought_amount / NULLIF(t.token_sold_amount, 0)  -- SOL per JitoSOL
      ELSE t.token_sold_amount / NULLIF(t.token_bought_amount, 0)   -- SOL per JitoSOL
    END) as price_ratio
  FROM dex.trades t
  ...
  GROUP BY es.epoch, hour
)
SELECT
  epoch,
  COUNT(DISTINCT hour) as hours_with_trades,
  AVG(price_ratio) as avg_price_ratio,
  STDDEV(price_ratio) as epoch_volatility,
  MAX(price_ratio) - MIN(price_ratio) as epoch_price_range,
  (MAX(price_ratio) - MIN(price_ratio)) / AVG(price_ratio) * 100 as epoch_range_pct,
  -- IL calculation inputs
  MIN(price_ratio) as epoch_min_price,
  MAX(price_ratio) as epoch_max_price,
  -- IL approximation for constant product AMM
  -- IL = 2 * sqrt(price_ratio) / (1 + price_ratio) - 1
  (2 * SQRT(MAX(price_ratio) / MIN(price_ratio)) / (1 + MAX(price_ratio) / MIN(price_ratio)) - 1) * 100 as estimated_il_pct
FROM hourly_prices
GROUP BY epoch
ORDER BY epoch;
